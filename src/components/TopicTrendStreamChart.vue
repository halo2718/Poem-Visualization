  ...
  setup(props) {
    ...
    onMounted(() => {
      ...
      axios.get('/topic_trend.json').then((resp) => {
        trendData.value = Object.fromEntries(
      ...
      watch([() => trendData.value, () => props.selectedTopics], () => {
        ...
        const mainGroup = svg.append('g').attr('width', '100%').attr('height', '100%');
        ...
        const xlist = ['XianQin', 'Qin', 'Han', 'WeiJin', 'NanBei', 'Sui', 'Tang',
             'Song', 'Liao', 'Jin', 'Yuan', 'Ming', 'Qing', 'Jindai', 'Dangdai']
        
        const xticklist = {'XianQin': '先秦', 'Qin': '秦', 'Han': '汉', 'WeiJin': '魏晋', 'NanBei': '南北', 
        'Sui': '隋', 'Tang': '唐', 'Song': '宋', 'Liao': '辽', 'Jin': '金', 'Yuan': '元', 'Ming': '明', 
        'Qing': '清', 'Jindai': '近代', 'Dangdai': '当代'}
        ...
        var mouseover = function(d) {
          ...
          d3.selectAll("#stream").style("opacity", .5)
          d3.select(d.target)
            .style("opacity", 1)
        }
